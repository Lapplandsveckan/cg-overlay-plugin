...
function loadBooks(): {name: string; abbr: string}[] {
    ...
    booksCache = toArray(parsed.bible.testament)
        ...
        .map((b: any) => ({name: String(b['#text']), abbr: String(b['@_abbr'])}));
    return booksCache;
}

function loadTranslation(file: string): any {
    if (translationCache.has(file)) return translationCache.get(file);
    const xml = fs.readFileSync(path.join(DATA_DIR, 'translations', `${file}.xml`), 'utf-8');
    const parsed = parser.parse(xml);
...
function parseNotation(notation: string): [number, number, number[]] {
    ...
    const [bookAndChapterRaw, versesRaw] = notation.split(':').map(s => s.trim());
    ...
    const bookIndex = books.findIndex(b =>
        b.name.includes(bookName) || b.abbr.includes(normalizedBook),
    );
...
function lookupVerses(
    ...
    const books = toArray(bible.bible.testament).flatMap((t: any) => toArray(t.book));
    const book = books.find((b: any) => String(b['@_number']) === String(bookNum));
    if (!book) throw new Error(`Book number ${bookNum} not found in ${translationFile}`);

    const chapters = toArray(book.chapter);
    const chapter = chapters.find((c: any) => String(c['@_number']) === String(chapterNum));
...
function normalizeNotation(bookNum: number, chapterNum: number, verseNumbers: number[]): string {
    const books = loadBooks();
...
function smartSplitText(text: string, maxLength: number, threshold = 0.75): string[] {
    ...
    const addChunk = (chunk: string) => { if (chunk.trim()) chunks.push(chunk.trim()); };

    for (const sentence of sentences) {
        buffer = `${buffer} ${sentence}`.trim();
        while (buffer.length > threshold * targetLength) {
            if (buffer.length <= targetLength) { addChunk(buffer); buffer = ''; break; }
...
export function getVerseSlides(lookup: VerseLookup): VerseSlide[] {
    ...
    if (!translationFile) throw new Error(`Unknown translation: "${lookup.translation}"`);

    const rangeStr = lookup.verseStart === lookup.verseEnd
    ...
    const verses = lookupVerses(translationFile, bookNum, chapterNum, verseNumbers);
