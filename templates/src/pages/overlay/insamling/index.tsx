...
import {
    formatNumber,
    useAnimatedNumbers,
} from '../../../lib/overlay/insamling/numbers';

export const InsamlingAnimation: React.FC<{
    state: number;
    goal: number;
    now: number;
}> = ({ state, goal, now }) => {
    ...
    return (
        <CG
            state={state}
            handle={(tl, state, _, styles) =>
                handleState(tl, state, styles, goal, now)
            }
            labels={['start', 'end']}
        ...
        >
            <div className={styles.diagram__main}>
                ...
                <img
                    ...
                    src="/images/Lappis_NEG_v1.1.png"
                    alt="Logo"
                />
